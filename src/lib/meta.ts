...
export async function exchangeCodeForToken(
  ...
  const data = await res.json()
  if (!res.ok || data.error) throw new Error(data.error?.message || 'Falha no OAuth')
...
export async function getLongLivedToken(
  ...
  const data = await res.json()
  if (!res.ok || data.error)
    throw new Error(data.error?.message || 'Falha ao obter token de longa duracao')
  return { accessToken: data.access_token, expiresIn: data.expires_in }
}
...
export function verifyMetaSignature(
  ...
  const expected =
    'sha256=' +
    crypto.createHmac('sha256', metaConfig.appSecret).update(rawBody, 'utf8').digest('hex')
...
export function leadDataToContact(fieldData: Record<string, string>): {
  ...
  const first = fieldData['first_name'] || fullName.split(' ')[0] || 'Lead Meta'
  const last =
    fieldData['last_name'] || fullName.split(' ').slice(1).join(' ') || undefined
