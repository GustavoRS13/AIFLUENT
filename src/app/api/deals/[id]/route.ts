...
export async function PATCH(
  ...
  try {
    ...
    if (existing.lead.organizationId !== orgId)
    ...
    if (parsed.data.expectedCloseAt)
      data.expectedCloseAt = new Date(parsed.data.expectedCloseAt as string);
          ...
          title:
            parsed.data.status === "won"
              ...
              : parsed.data.status === "lost"
                ? "Negocio perdido"
                : "Negocio atualizado",
          description: parsed.data.value
            ? `Valor: R$${parsed.data.value}`
            : undefined,
    ...
    // Automação: negócio GANHO → lead ganho → transfere ao departamento Educacional
    if (parsed.data.status === "won" && existing.leadId) {
      await prisma.lead
        .update({
      ...
      await transferWonLeadToEducational(prisma, existing.leadId, orgId);
    }
    ...
    return NextResponse.json(
      { error: "Falha ao atualizar negocio" },
      { status: 500 },
    );
  }
}
...
export async function GET(
  ...
  try {
    ...
    if (!deal || deal.lead.organizationId !== orgId)
      return NextResponse.json(
        { error: "Negocio nao encontrado" },
        { status: 404 },
      );
