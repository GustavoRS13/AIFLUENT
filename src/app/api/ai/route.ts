...
// Dias desde o ÚLTIMO contato real (lastContactAt ou última mensagem da conversa).
// Retorna null quando NUNCA houve contato — evita o valor fake "999".
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function contactDays(lead: any): number | null {
  const candidates = [
    lead?.lastContactAt,
    lead?.conversations?.[0]?.lastMessageAt,
    lead?.conversations?.[0]?.lastInboundAt,
  ].filter(Boolean);
  if (!candidates.length) return null;
  const latest = Math.max(
    ...candidates.map((d: string) => new Date(d).getTime()),
  );
...
export async function POST(request: NextRequest) {
  ...
  if (action === "generate-campaign-message") {
    const templates =
      campaignTemplates[channel || "whatsapp"] || campaignTemplates.whatsapp;
  ...
  if (action === "score-lead") {
    const factors = {
      ...
      recencyBonus: context?.daysSinceLastContact
        ? Math.max(0, 3 - context.daysSinceLastContact * 0.1)
        : 0,
    };

    const base =
      factors.sourceScore[
        context?.source as keyof typeof factors.sourceScore
      ] || 5;
    const temp =
      factors.temperatureScore[
        context?.temperature as keyof typeof factors.temperatureScore
      ] || 5;
    const score = Math.min(
      10,
      (base + temp + factors.activityBonus + factors.recencyBonus) / 3,
    );
    ...
    return NextResponse.json({
      ...
      recommendation:
        score >= 7
          ? "Lead quente — priorizar contato imediato"
          : score >= 4
            ? "Lead morno — manter na sequência de follow-up"
  ...
  if (action === "suggest-follow-up") {
    const suggestions = [
      {
        time: "2h",
        message: "Enviar mensagem de agradecimento pelo interesse",
      },
      {
        time: "24h",
        message: "Compartilhar material sobre o curso de interesse",
      },
  ...
  if (action === "suggest-next-step") {
    ...
    const dsc = contactDays(lead);
    const daysSinceContact = dsc ?? 0;
    const semContatoTxt =
      dsc == null ? "sem contato registrado" : `sem contato ha ${dsc} dias`;
    ...
    if (lead.temperature === "hot" && (lead.aiScore ?? 0) > 70) {
      ...
      suggestion = `${lead.firstName} esta ${semContatoTxt}.`;
      if (recentNotes > 0) {
      ...
      suggestion = `${lead.firstName} esta frio e ${semContatoTxt}. Considere uma campanha de reativacao ou remover do pipeline.`;
  ...
  if (action === "summarize-conversation") {
    ...
    const stageChanges = lead.activities.filter(
    ...
    if (dealValue > 0)
      summary += `, valor total R$${dealValue.toLocaleString("pt-BR")}`;
    summary += `. ${noteCount} notas, ${stageChanges} mudancas de estagio.`;
    if (callCount > 0) summary += ` ${callCount} ligacoes.`;
    if (lastNote?.description)
      summary += ` Ultima nota: "${lastNote.description.substring(0, 80)}${lastNote.description.length > 80 ? "..." : ""}"`;
  ...
  if (action === "estimate-probability") {
    ...
    if (lead.temperature === "hot") {
      probability += 25;
      factors.temperature = "Quente: +25%";
    } else if (lead.temperature === "warm") {
      probability += 10;
      factors.temperature = "Morno: +10%";
    } else {
      probability -= 10;
      factors.temperature = "Frio: -10%";
    }

    // Score factor
    if (lead.aiScore && lead.aiScore > 70) {
      probability += 15;
      factors.score = `Score ${lead.aiScore}%: +15%`;
    } else if (lead.aiScore && lead.aiScore > 40) {
      probability += 5;
      factors.score = `Score ${lead.aiScore}%: +5%`;
    } else {
      factors.score = `Score ${lead.aiScore ?? 0}%: +0%`;
    }
    ...
    if (actCount >= 10) {
      probability += 10;
      factors.engagement = `${actCount} atividades: +10%`;
    } else if (actCount >= 5) {
      probability += 5;
      factors.engagement = `${actCount} atividades: +5%`;
    } else {
      factors.engagement = `${actCount} atividades: +0%`;
    }
    ...
    const stageIdx = stageNames.findIndex((s) =>
      lead.stage?.name?.toLowerCase().includes(s.toLowerCase()),
    );
    if (stageIdx >= 3) {
      probability += 15;
      factors.stage = `${lead.stage?.name}: +15%`;
    } else if (stageIdx >= 1) {
      probability += 5;
      factors.stage = `${lead.stage?.name}: +5%`;
    } else {
      factors.stage = `${lead.stage?.name || "Inicio"}: +0%`;
    }

    // Deal factor
    if (lead.deals.some((d) => d.status === "open")) {
      probability += 5;
      factors.deal = "Negocio aberto: +5%";
    }
    ...
    return NextResponse.json({
      ...
      recommendation:
        probability >= 70
          ? "Alta chance — priorizar"
          : probability >= 40
            ? "Chance moderada — nutrir"
            : "Baixa chance — reavaliar",
    });
  }
  ...
  if (action === "detect-risk") {
    ...
    if (lead.temperature === "cold")
      risks.push({
        signal: "Temperatura fria — lead pode estar desinteressado",
        severity: "high",
      });
    if (dsc != null && dsc > 7)
      risks.push({
        signal: `Sem contato ha ${dsc} dias`,
        severity: dsc > 14 ? "high" : "medium",
      });
    if (lead.deals.some((d) => d.status === "lost"))
      risks.push({ signal: "Negocio anterior perdido", severity: "medium" });
    if ((lead.aiScore ?? 0) < 30)
      risks.push({
        signal: `Score muito baixo (${lead.aiScore ?? 0}%)`,
        severity: "medium",
      });
    if (lead.activities.length === 0)
      risks.push({
        signal: "Nenhuma atividade recente registrada",
        severity: "low",
      });
    if (lead.lostReason)
      risks.push({
        signal: `Motivo de perda anterior: ${lead.lostReason}`,
        severity: "high",
      });

    const level = risks.some((r) => r.severity === "high")
      ? "high"
  ...
  if (action === "full-analysis") {
    ...
    const insights = await getAIInsights({
      ...
      lastNote:
        lead.activities.find((a) => a.type === "note")?.description || null,
  ...
  if (action === "generate-response") {
    ...
    if (!lead)
      return NextResponse.json({ response: "Ola! Como posso ajudar?" });
